...
@testset "Heat Exchanging" begin
    ...
    @testset "Radiator design and off-design performance" begin
        ...
        # mirror bare-pare radiator writes to typed state (tasopt-keh: HXPort now reads eng.RadCoolantT/P/Qradiator)
        for im in 1:length(ac.missions), ip in 1:length(ac.missions[im].points)
            ac.missions[im].points[ip].engine.RadCoolantT = 453.15
            ac.missions[im].points[ip].engine.RadCoolantP = 300000.0
