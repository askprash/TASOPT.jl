...
function wireAeroControls() {
  const sweepSelect = document.getElementById("aero-sweep-field") as HTMLSelectElement | null;
  if (!sweepSelect) {
    return;
  }
  sweepSelect.addEventListener("change", (event) => {
    state.aeroSweepField = (event.target as HTMLSelectElement).value;
    renderTab();
  });
}
...
function parseAdvancedCompareText(text, data = state.data) {
  ...
  const valid: { expression: string; display: string }[] = [];
