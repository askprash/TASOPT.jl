...
function plotOverviewTab() {
  missionPlot(
    "overview-efficiency",
    "(L/D) / TSFC",
    (aircraft) => aircraft.mission.lod.map((lod, point) => {
...
function plotWeightsTab() {
  ...
  const structureTraces = state.data.aircraft.map((aircraft, index) => barTrace(
    labels,
    aircraft.weights.structure_breakdown.values_t,
    aircraft.name,
    index,
  ));
...
function plotEngineTab() {
  ...
  const ratioTraces = state.data.aircraft.flatMap((aircraft, index) => ([
    ...
    missionTrace(
      aircraft,
      aircraft.engine.opr.map((value) => value / 10),
      `${aircraft.name} OPR / 10`,
      index,
      {
        mode: "lines",
