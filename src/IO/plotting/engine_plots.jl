...
These functions accept an [`engine.SweepResult`](@ref) returned by
[`engine.run_engine_sweep`](@ref) and return a `Plots.Plot` object that
can be displayed or saved with `savefig`.
"""
...
gas path for every mission point in `result`.

One line is drawn per mission point; colours follow the default Plots.jl
palette and labels are taken from `result.ip_labels`.

Only stations with a meaningful total-state entry in `pare` are shown
(freestream through fan nozzle, 13 stations total).  Heat-exchanger
intermediate stations (19c, 25c, 4a, 49c) and static-only stations (6, 8)
are excluded because they are all-zero for the default turbofan.
...
p = TASOPT.plot_engine_station_profiles(sweep)
...
**Always included** (four panels, data from `result`):
...
When `ac` is provided the corrected spool speeds are read directly from
`ac.pare` for each mission point in `result`.
...
`kwargs` are forwarded to the outer `Plots.plot` call.
...
p_basic = TASOPT.plot_engine_performance(sweep)

# Full seven-panel plot (with spool speeds)
p_full = TASOPT.plot_engine_performance(sweep; ac=ac)
...
function plot_engine_performance(result::engine.SweepResult;
    ...
    # Convert TSFC from kg/N/s to mg/N/s for more readable y-axis values
    ...
    # Spool speeds: extract corrected values from pare
    ...
    # 7 panels: layout (4,2) leaves panel 8 empty — acceptable for this figure
