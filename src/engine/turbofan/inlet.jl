...
|:--------------------|:-----|:-------------------------------------------------------|
| `pid`               | —    | Diffuser total-pressure ratio `pt1_8 / pt0`, typically 0.97–1.0 |
...
2. **BLI entropy mixing (18 → 2/19):** The airframe boundary layer, whose
   ...
   pt2    = pt1_8 × exp(−sbfan)
   pt1_9   = pt1_8 × exp(−sbcore)    # sbcore = sbfan if eng_has_BLI_cores, else 0
   Tt2    = Tt1_9 = Tt1_8            # adiabatic BLI
   ```
...
mutable struct Inlet{T<:AbstractFloat}
    pid              ::T    # diffuser total-pressure ratio  pt1_8/pt0   [—]
...
"""
    inlet_diffuser!(st1_8, st0, inlet) -> st1_8
...
Returns `st1_8` (mutated in place).
"""
function inlet_diffuser!(
    ...
    st1_8.Rt    = st0.Rt
    st1_8.st    = st0.st
    st1_8.alpha = st0.alpha

    # Pressure recovery
    st1_8.pt = st0.pt * inlet.pid
...
"""
    inlet_bli_mixing!(st2, st1_9, st1_8, st0, inlet,
...
| `st1_9`    | —      | LPC-inlet station (mutated)                                  |
| `st1_8`    | —      | Inlet-face station (read-only)                               |
...
ingested flow:
    ...
    sbfan  = Kinl·γ₀ / (ṁ_mix·a₂²)
    pt2    = pt1_8·exp(−sbfan)
    pt1_9   = pt1_8·exp(−sbcore)    # sbcore = sbfan or 0
...
function inlet_bli_mixing!(
    ...
    for st_out in (st2, st1_9)
        st_out.Tt    = st1_8.Tt
        st_out.ht    = st1_8.ht
        st_out.cpt   = st1_8.cpt
        st_out.Rt    = st1_8.Rt
        st_out.st    = st1_8.st
        st_out.alpha = st1_8.alpha
    end
    ...
    # Freestream reference factors (avoiding circular dependency on pt2/pt1_9)
    pt0  = st0.pt
    Tt0  = st0.Tt
    ...
    st2.pt  = st1_8.pt * exp(-sbfan)
    st1_9.pt = st1_8.pt * exp(-sbcore)
